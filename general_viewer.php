...
try {
    if ($studyId > 0) {
        $stmt = db()->prepare("
            SELECT st.*, p.patient_name, p.patient_id AS pid, p.birth_date, p.sex, p.id AS patient_db_id
            FROM studies st
        ...
        $seriesStmt = db()->prepare("
            SELECT se.*, st.id AS parent_study_id, st.study_date, st.modality
            FROM series se
...
.reports-wrap{
  ...
  max-height: 34vh;
  overflow: auto;
  border: 1px solid rgba(54,111,209,.12);
  border-radius: 12px;
  background: linear-gradient(180deg,#0f1724,#0d1522);
}
.reports-head{
  padding: 10px 12px;
  border-bottom: 1px solid rgba(54,111,209,.1);
  color: #9ec3e6;
  font-weight: 700;
  letter-spacing: .04em;
}
.report-card{background:#0f172a;padding:5px;margin:12px}
...
@media (max-width: 900px){
  ...
  .pacs-loader-center{ margin-top: 34px; transform: scale(.83); }
  .pacs-loader-text{ margin-top: 6px; }
  .pacs-progress-wrap{ margin-top: 10px; width: 86vw; }

  .reports-wrap{max-height:28vh;margin:8px}
  .report-card{margin:8px}
}
...
.report-modal-box{
  ...
  .report-header{
    ...
    display: block !important; /* stack logo + patient box */
  }
...
<div class="reports-wrap" style="margin:10px 12px 0;max-height:34vh;overflow:auto;border:1px solid rgba(54,111,209,.12);border-radius:12px;background:linear-gradient(180deg,#0f1724,#0d1522);">
  <?php foreach ($reports as $rp):
    $text = $rp['report_text'] ?? '';
    $isHtml = (stripos($text, '<img ') !== false || stripos($text, '<br') !== false || stripos($text, '<p>') !== false);
  ?>
    <div class="report-card" style="background:#0f172a;padding:5px;margin:12px">
...
<div id="viewerShell" class="viewer-shell">
  <header class="viewer-header">
    <div class="vh-left">
      <a href="<?= $viewerMode === 'patient_type' ? 'general_patient.php?patient='.(int)$patientId : 'study_list.php' ?>" class="vh-back">
    ...
    <div class="vh-tools" id="toolbarMain">
      ...
      <div class="tool-sep"></div>
      <div class="tool-group">
        <button class="tool-btn" data-tool="Pan" title="Pan"><i class="fas fa-hand-paper"></i></button>
        <button class="tool-btn" data-tool="Zoom" title="Zoom"><i class="fas fa-search-plus"></i></button>
        <button class="tool-btn" data-tool="Wwwc" title="Window"><i class="fas fa-adjust"></i></button>
