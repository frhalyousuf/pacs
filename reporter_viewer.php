...
try {
    if ($studyId > 0) {
        ...
        try {
            ...
            $authStmt->execute([$rep['username'], $study['pid']]);
            if (!$authStmt->fetch()) {
        ...
        // Keep patientId available for reporter back-link flow.
        if ($patientId <= 0 && !empty($study['patient_id'])) {
    ...
    // Fetch Patient Data & Reports
    if ($patientId > 0) {
        $pStmt = db()->prepare("SELECT id, patient_name, patient_id, birth_date, sex FROM patients WHERE id = ? LIMIT 1");
        $pStmt->execute([$patientId]);
        $patient = $pStmt->fetch();

        $rpt = db()->prepare("
            SELECT pr.id, pr.report_text, pr.study_id, pr.created_at, pr.updated_at, r.full_name AS reporter_name
...
// Demographics for the report header — patient row first, study row as fallback.
 $patientName = $patient['patient_name'] ?? ($study['patient_name'] ?? '—');
 $patientPid  = $patient['patient_id']   ?? ($study['pid'] ?? '—');
 $patientSex  = $patient['sex']          ?? ($study['sex'] ?? '—');
 $birthDate   = $patient['birth_date']   ?? ($study['birth_date'] ?? null);
...
// Where the back arrow goes. Reporters always arrive from reporter_patient.php.
 $backUrl = $patientId > 0
    ? 'reporter_patient.php?patient=' . (int)$patientId
    : 'reporter_index.php';
?>
<!doctype html>
<html lang="en">
...
/* Back link + patient identity (reporter panel only) */
.vh-back{width:40px;height:40px;border-radius:12px;display:inline-flex;align-items:center;justify-content:center;text-decoration:none;color:var(--c-text);border:1px solid var(--c-border);background:rgba(255,255,255,.02);flex:0 0 auto;transition: all 0.2s cubic-bezier(0.18, 0.89, 0.32, 1.28);}
.vh-back:hover{background:rgba(var(--c-accent-rgb),.08);border-color:var(--c-accent);color:var(--c-accent);transform:translateY(-2px)}
.vh-patient{display:flex;flex-direction:column;gap:2px;min-width:0}
.vh-name{font-size:15px;font-weight:700;color:var(--c-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:190px}
.vh-meta{color:var(--c-accent);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:190px}
...
@media (max-width: 900px){
  ...
  .vh-back{width:34px;height:34px;border-radius:10px}
  .vh-name{font-size:13px;max-width:120px}
  .vh-meta{font-size:10px;max-width:120px}
...
body[data-mode="light"] .tool-btn:hover,
body[data-mode="light"] .vh-back:hover{background:rgba(var(--c-accent-rgb),.12)}
...
<div id="viewerShell" class="viewer-shell">
  <header class="viewer-header">
    <div class="vh-left">
      <!-- Back to the reporter's patient page (reporter panel entry point) -->
      <a href="<?= htmlspecialchars($backUrl) ?>" class="vh-back" title="Back to patient"><i class="fas fa-arrow-left"></i></a>

      <div class="vh-patient">
        <span class="vh-name"><?= htmlspecialchars($study['patient_name'] ?? '—') ?></span>
        <span class="vh-meta">ID: <?= htmlspecialchars($study['pid'] ?? '—') ?></span>
      </div>
          ...
          <tr><td>
            <div class="report-header">
              ...
              <div class="report-patient-demographics">
                ...
                <span class="demo-value"><?= htmlspecialchars($patientName) ?></span>
                <span class="demo-label">Age/Sex:</span>
                <span class="demo-value"><?= htmlspecialchars($patientAge) ?> / <?= htmlspecialchars($patientSex) ?></span>
                <span class="demo-label">Patient ID:</span>
                <span class="demo-value"><?= htmlspecialchars($patientPid) ?></span>
...
<!-- ============================================================
     SCRIPT 1: UI DROPDOWN LOGIC & THEME SWITCHER
     ============================================================ -->
<script>
// 1. Dropdown Logic
...
// 4. Background Preloader Logic (Preloads 1st Series, then all remaining)
(function() {
...
(function () {
  ...
  ensureToolsRegistered();
  console.log('[Viewer] measurement tools + settings + clipboard screenshot + dropdown themes ready');
})();
